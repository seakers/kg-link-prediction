...
from torch._C_.nn import linear
...
from nn.init import constant_, xavier_normal_, xavier_uniform_
from nn.modules.linear import NonDynamicallyQuantizableLinear
...
class RotTransformer(nn.module):
    ...
    def context_subgraph(self, anchor: Entity) : #-> [([], int)]:
        """
...
class MultiheadAttentionUniqueBias(nn.Module):
    ...
    def forward(
        ...
        if not self._qkv_same_embed_dim:
            attn_output, attn_output_weights = self.multi_head_attention_relation_bias_forward(
                query,
                key,
                value,
                self.embed_dim,
    ...
    def multi_head_attention_relation_bias_forward(
        self,
        query: Tensor,
        key: Tensor,
        value: Tensor,
        ...
        if need_weights:
            ...
            attn_output = self.scaled_dot_product_attention_relation_bias(
    ...
    def scaled_dot_product_attention_relation_bias(query, key, value, relation_bias, attn_mask=None, dropout_p=0.0,
